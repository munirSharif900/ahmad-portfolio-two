...
const filters = ["All", "Unread", "query_submitted", "project_created", "project_updated", "testimonial_created", "testimonial_updated", "service_created", "service_updated"];
const filterLabels: Record<string, string> = {
  All: "All", Unread: "Unread",
  query_submitted: "Queries", project_created: "Projects", project_updated: "Projects Updated",
  testimonial_created: "Testimonials", testimonial_updated: "Testimonials Updated",
  service_created: "Services", service_updated: "Services Updated",
};

export default function NotificationsView() {
  const { notifications, unreadCount, activeFilter, loading, setActiveFilter, markRead, markAllRead, deleteNotification, fetchNotifications } = useNotificationStore();
  ...
  const filtered = notifications.filter(n => n && typeof n.id === 'number').filter((n) => {
  ...
  return (
    <div className="space-y-6">
        ...
        {filters.map((f) => (
          <button key={f} onClick={() => setActiveFilter(f)}
            ...
            {f === "Unread" && unreadCount > 0 && (
              <span className="ml-1.5 bg-violet-500 text-white text-[10px] px-1.5 py-0.5 rounded-full">{unreadCount}</span>
            )}
          </button>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden divide-y divide-gray-100 dark:divide-gray-800/60">
        ...
        {filtered.map((n, idx) => {
          const cfg = typeConfig[n.type] ?? { bg: "bg-gray-800", text: "text-gray-400", icon: null };
          return (
            <div key={`${n.id}-${idx}`}
              className={`flex items-start gap-4 px-5 py-4 group transition-colors ${!n.is_read ? "bg-violet-50 dark:bg-gray-900/60" : "hover:bg-gray-50 dark:hover:bg-gray-900/30"}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="text-xs text-gray-400 dark:text-gray-600 whitespace-nowrap flex-shrink-0">{new Date(n.created_at).toLocaleString()}</span>
                </div>
