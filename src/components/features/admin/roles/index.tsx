...
import { getAllUsers, createUser, updateUser, deleteUser, revokeUser, RoleUserAPI } from "@/src/api/services/roles";
...
function parseError(e: any, fallback: string): string {
  ...
  return Object.entries(data).map(([f, errs]) => `${f}: ${Array.isArray(errs) ? errs.join(", ") : errs}`).join(" | ") || fallback;
}
...
export default function RolesView() {
  ...
  function fetchUsers(p: number) {
    ...
    if (statusFilter !== "All Status") params.is_active = statusFilter === "Active";
    getAllUsers(params)
      .then(data => {
  ...
  async function handleAdd() {
    ...
    try {
      ...
      setModal(null); fetchUsers(page);
    } catch (e: any) { toast.error(parseError(e, "Failed to create user")); }
  ...
  async function handleSaveEdit() {
    ...
    try {
      ...
      setModal(null); fetchUsers(page);
    } catch (e: any) { toast.error(parseError(e, "Failed to update user")); }
    finally { setSaving(false); }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setModal(null); fetchUsers(page);
    } catch (e: any) { toast.error(parseError(e, "Failed to delete user")); }
    finally { setSaving(false); }
  }
  ...
  const formatDate = (d: string) => d ? new Date(d).toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" }) : "—";
      ...
      <div className="bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden">
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800/60">
            {loading && <tr><td colSpan={6} className="py-16 text-center text-gray-400 text-sm">Loading...</td></tr>}
            {!loading && displayUsers.length === 0 && <tr><td colSpan={6} className="py-16 text-center text-gray-400 text-sm">No users found</td></tr>}
              ...
              <tr key={u.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/40 transition-colors">
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center gap-3">
                    <button onClick={() => setModal({ type: "view", user: u })} title="View" className="text-gray-400 hover:text-violet-400 transition-colors">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                    </button>
                    <button onClick={() => openEdit(u)} title="Edit" className="text-gray-400 hover:text-blue-400 transition-colors">
                    ...
                    <button onClick={() => setModal({ type: "status", user: u })} title="Toggle Status" className="text-gray-400 hover:text-emerald-400 transition-colors">
                    ...
                    <button onClick={() => setModal({ type: "delete", user: u })} title="Delete" className="text-gray-400 hover:text-red-400 transition-colors">
        ...
        <Pagination page={page} totalPages={totalPages} totalCount={totalCount} pageSize={PAGE_SIZE} loading={loading} onPageChange={setPage} label="users" />
      </div>

      {modal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setModal(null)}>
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            {modal.type === "view" && (
              <div className="p-6 space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-sm items-center">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${roleBadge[modal.user.role] ?? "bg-gray-800 text-gray-300 border border-gray-700"}`}>{roleLabel[modal.user.role] ?? modal.user.role}</span>
                  </div>
                  <div className="flex justify-between text-sm items-center">
                ...
                <button onClick={() => setModal(null)} className="w-full py-2 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-sm font-medium hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">Close</button>
              </div>
            )}
            {(modal.type === "add" || modal.type === "edit") && (
              <div className="p-6 space-y-4">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">{modal.type === "add" ? "Add New User" : "Edit User"}</h3>
                ...
                <div className="flex gap-3 pt-1">
                  ...
                  <button onClick={modal.type === "add" ? handleAdd : handleSaveEdit} disabled={saving} className="flex-1 py-2 rounded-lg bg-violet-600 hover:bg-violet-700 disabled:opacity-60 text-white text-sm font-medium transition-colors">
                    {saving ? "Saving..." : modal.type === "add" ? "Add User" : "Save Changes"}
                  </button>
                </div>
              </div>
            )}
            {modal.type === "status" && (
              <div className="p-6 space-y-4 text-center">
                <div className="w-14 h-14 rounded-full bg-emerald-900/30 flex items-center justify-center mx-auto">
                  <svg className="w-7 h-7 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                </div>
                <div>
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">Toggle Status</h3>
                  <p className="text-sm text-gray-500 mt-1">Change <span className="text-white font-medium">{modal.user.name}</span>'s status from{" "}
                    <span className={`font-medium ${modal.user.is_active ? "text-emerald-400" : "text-gray-400"}`}>{modal.user.is_active ? "Active" : "Revoked"}</span> to{" "}
                    <span className={`font-medium ${modal.user.is_active ? "text-gray-400" : "text-emerald-400"}`}>{modal.user.is_active ? "Revoked" : "Active"}</span>?
                  </p>
                </div>
                <div className="flex gap-3">
                  <button onClick={() => setModal(null)} className="flex-1 py-2 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-sm font-medium hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">Cancel</button>
                  <button onClick={handleToggleStatus} disabled={saving} className="flex-1 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 text-white text-sm font-medium transition-colors">{saving ? "Updating..." : "Confirm"}</button>
                </div>
              </div>
            )}
            {modal.type === "delete" && (
              <div className="p-6 space-y-4 text-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">Are you sure you want to delete <span className="text-white font-medium">{modal.user.name}</span>? This cannot be undone.</p>
                </div>
                <div className="flex gap-3">
                  <button onClick={() => setModal(null)} className="flex-1 py-2 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-sm font-medium hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">Cancel</button>
                  <button onClick={handleDelete} disabled={saving} className="flex-1 py-2 rounded-lg bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white text-sm font-medium transition-colors">{saving ? "Deleting..." : "Delete"}</button>
                </div>
              </div>
            )}
          </div>
...
function FormFields({ form, setForm }: { form: { name: string; email: string; role: string }; setForm: React.Dispatch<React.SetStateAction<typeof form>> }) {
  return (
    <div className="space-y-3">
